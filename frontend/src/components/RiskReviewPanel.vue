      ...
      <div class="review-main-column">
        ...
        <div v-if="review?.findings?.length" class="finding-list">
          <article v-for="finding in review.findings" :key="finding.id" class="finding-card" :class="finding.status.toLowerCase()">
            ...
            <details v-if="finding.retrieval_candidates?.length" class="retrieval-candidates">
              <summary>
                <span><FileSearch :size="15" />本项检索候选</span>
                <span class="candidate-summary-meta">
                  <small>合同 {{ candidatesFor(finding, 'CONTRACT').length }} 条 · 制度 {{ candidatesFor(finding, 'POLICY').length }} 条</small>
                  <i class="candidate-expand-label"></i>
                  <ChevronDown :size="14" />
                </span>
              </summary>
...
.review-hero h1 { margin: 0; color: var(--navy); font-family: Georgia, 'Noto Serif SC', serif; font-size: 20px; line-height: 1.08; letter-spacing: -.032em; }
...
.candidate-summary-meta { margin-left: auto; color: #849088; font-weight: 400 !important; }.candidate-expand-label { padding-left: 8px; border-left: 1px solid #dce4dd; color: var(--green); font-size: 8px; font-style: normal; font-weight: 700; }.candidate-expand-label::before { content: '展开候选'; }.retrieval-candidates[open] .candidate-expand-label::before { content: '收起候选'; }.candidate-summary-meta svg { transition: transform .2s ease; }.retrieval-candidates[open] .candidate-summary-meta svg { transform: rotate(180deg); }
